...
export default function Word() {
  ...
  const StartGuessWord = () => {
    ...
    const answer: WordDataType = WordData[answerIndex];
  ...
  const GuessAreaTemplate = () => {
    let content: string | null = "點擊按鈕出題";
    if (Answer?.chi) {
      switch (GuessIndex) {
        case "word":
          content = Answer.word;
  ...
  // 檢查答案
  const CheckAnswer = ({ option, category }: { option?: WordDataType, category?: number }) => {
    // 判斷 word、kana
        ...
        <div className="flex items-center">
          <div className="form-control mx-2">
            <label className="label cursor-pointer">
              ...
              />
              <span className="label-text  mx-2">動詞</span>
            </label>
          </div>
          <div className="form-control mx-2">
            <label className="label cursor-pointer">
          ...
          <label className="label cursor-pointer">
            <input
              ...
              id="GuessCategory"
              className="radio  "
              onChange={() => setGuessIndex("category")}
              checked={GuessIndex == "category"}
            />
            <span className="label-text  mx-2">第幾類</span>
          </label>
        </div> */}
        </div>
      </div>
          ...
          <div className="form-control mx-2">
            <label className="label cursor-pointer">
              <input
                ...
                name="Answer"
                className="radio  "
                onChange={() => setAnsertIndex("category")}
                checked={AnswerIndex == "category"}
              />
              <span className="label-text  mx-2">幾類</span>
            </label>
          </div>
        </div>
      </div>
        ...
        {Options && Options.length > 0 ? (
          <div>
            {AnswerIndex !== "category" ? (
              ...
              <div>
                <div
                  className="m-2 p-2 border-2 text-2xl text-center font-bold hover:border-black hover:bg-gray-200"
                  onClick={() => CheckAnswer({ category: 1 })}
                >
                  第一類
                </div>
                <div
                  className="m-2 p-2 border-2 text-2xl text-center font-bold hover:border-black hover:bg-gray-200"
                  onClick={() => CheckAnswer({ category: 2 })}
                >
                  第二類
                </div>
                <div
                  className="m-2 p-2 border-2 text-2xl text-center font-bold hover:border-black hover:bg-gray-200"
                  onClick={() => CheckAnswer({ category: 3 })}
                >
                  第三類
                </div>
              </div>
